    ...
    * `name`: the name of the artist
    ...
    if type == 'tracks':
        with db.engine.begin() as conn:
            sql = """
            SELECT t1.track_id, t.title AS track_title, t.runtime, al.title AS album_title, ar.name
            FROM tracks AS t
            JOIN 
            (
            SELECT track_id
            ...
            """
            result = conn.execute(sa.text(sql), [{"name": name, "limit": limit}]).fetchall()
                    ...
                    "tracks": [
                        {
                            ...
                            "artist_names": {row.name for row in result if row.track_id == track.track_id}
                        }
                        for track in result
                    ]
                }
                    ...
                    "albums": [
                        {
                            ...
                            "title": album.title,
                            "artist_names": {row.name for row in result if row.album_id == album.album_id},
